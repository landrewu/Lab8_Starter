describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < items.length; i++) {
      const item = await items[i].getProperty('shadowRoot')
      const button = await item.$('button')
      await button.click()
    }
    const text = await page.$('#cart-count');
    const inner = await text.getProperty('innerText')
    expect(await inner.jsonValue()).toBe('20')
  }, 10000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const text = await page.$('#cart-count');
    const inner = await text.getProperty('innerText')
    expect(await inner.jsonValue()).toBe('0')
  }, 10000);
